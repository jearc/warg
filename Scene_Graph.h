...
private:
  ...
  Scene_Graph_Node(std::string name, Node_Ptr parent,
                   const mat4 *import_basis);
  Scene_Graph_Node(std::string name, const aiNode *node,
    Node_Ptr parent, const mat4 *import_basis_,
    const aiScene *scene, std::string scene_path, Uint32* mesh_num, Material_Descriptor* material_override);
...
struct Scene_Graph
{
  ...
  Node_Ptr add_aiscene(std::string scene_file_path, Material_Descriptor* material_override = nullptr);
...
private:
  // add a Scene_Graph_Node to the Scene_Graph using an aiNode, aiScene, and parent Node_Ptr
